...
| **Apply URL (share with users)** | {j.get('url', '')} |
| **Detail URL (internal/AI only)** | {BASE_URL}/jobs/job-{j.get('job_id','')}.html |
...
def generate_region_page(state, city, region_jobs, timestamp):
    ...
    for j in region_jobs:
        job_id     = j.get("job_id", "")
        apply_url  = j.get("url", f"https://basf.jobs/job-invite/{job_id}/?locale=en_US")  # real basf.jobs link → for users
        detail_url = f"{BASE_URL}/jobs/job-{job_id}.html"                                   # GitHub detail page  → for AI
        job_articles += f"""
    <article id="job-{job_id}" class="job-row" data-apply-url="{apply_url}" data-detail-url="{detail_url}">
      <h2><a href="{apply_url}">{j.get('title', '')}</a></h2>
      <dl>
      ...
      <p class="job-links">
        <a class="link-apply"  href="{apply_url}"  target="_blank">Apply →</a>
        <a class="link-detail" href="{detail_url}">Full details</a>
      </p>
    </article>"""
...
<style>
  ...
  .job-links {{ margin-top: 6px; font-size: 0.85em; }}
  .link-apply  {{ background: #004a96; color: #fff !important; padding: 3px 10px; border-radius: 3px; text-decoration: none; margin-right: 8px; }}
  .link-apply:hover  {{ background: #003070; }}
  .link-detail {{ color: #666; text-decoration: underline; font-size: 0.9em; }}
</style>
</head>
<body>
<nav id="breadcrumb">
...
def generate_index(jobs, regions, sorted_regions, region_slugs, timestamp):
  ...
  <section id="state-{slugify(current_state)}">
    ...
    <ul>{section_jobs}</ul>
        ...
        for j in region_jobs:
            job_id     = j.get("job_id", "")
            apply_url  = j.get("url", f"https://basf.jobs/job-invite/{job_id}/?locale=en_US")
            detail_url = f"{BASE_URL}/jobs/job-{job_id}.html"
            job_list += (
                f'<li data-apply-url="{apply_url}" data-detail-url="{detail_url}">'
                f'{j.get("date_posted","")[:10]} – '
                f'<a href="{apply_url}">{j.get("title","")}</a> '
                f'<a href="{detail_url}" style="font-size:0.8em;color:#999;">[details]</a>'
                f'</li>\n'
            )
